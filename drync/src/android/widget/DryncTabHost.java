 ...
 */
public class DryncTabHost extends FrameLayout implements ViewTreeObserver.OnTouchModeChangeListener {

    private DryncTabWidget mTabWidget;
    ...
    public DryncTabHost(Context context) {
        super(context);
        initTabHost();
    }

    public DryncTabHost(Context context, AttributeSet attrs) {
        super(context, attrs);
        initTabHost();
    }
...
mTabHost.addTab(TAB_TAG_1, "Hello, world!", "Tab 1");
      */
    public void setup() {
        mTabWidget = (DryncTabWidget) findViewById(R.id.tabs);
        if (mTabWidget == null) {
        ...
        ((DryncTabWidget) mTabWidget).setTabSelectionListener(new DryncTabWidget.OnTabSelectionChanged() {
    ...
    public void setCurrentTab(int index) {
        ...
        mCurrentTab = index;
        final DryncTabHost.TabSpec spec = mTabSpecs.get(index);
    ...
    public class TabSpec {
        ...
        private Queue<IntentContentStrategy> intentQueue = new LinkedList<IntentContentStrategy>();
        ...
        public TabSpec setContent(Intent intent) {
            mContentStrategy = new IntentContentStrategy(mTag, intent);
            intentQueue.add((IntentContentStrategy) mContentStrategy);
            return this;
        }
        ...
        public TabSpec replaceContent(Intent intent, boolean keepAlive)
        {        
        	if (!keepAlive)
        	{
        		mContentStrategy.tabClosed();
        		mLocalActivityManager.destroyActivity(this.getTag(), true);
        		this.intentQueue.poll();
        	}
        	
        	mContentStrategy = new IntentContentStrategy(mTag, intent);
        	this.intentQueue.add((IntentContentStrategy) mContentStrategy);
        ...
        public TabSpec stepContentBack()
        {
        	if (this.intentQueue.size() < 2)
        		return this;
        	...
        	IntentContentStrategy lastIntent = this.intentQueue.peek();
        	mContentStrategy = lastIntent;
        	mCurrentView = mContentStrategy.getContentView();
            if (mCurrentView.getParent() == null)
            {
                mTabContent.addView(mCurrentView, new ViewGroup.LayoutParams(ViewGroup.LayoutParams.FILL_PARENT, ViewGroup.LayoutParams.FILL_PARENT));
	...
	public TabSpec getCurrentTabSpec() {
		if (this.getCurrentTab() < this.mTabSpecs.size())
			return this.mTabSpecs.get(this.getCurrentTab());
		else
			return null;
	}
...
public class DryncTabHost extends TabHost {
	ArrayList<TabHost.TabSpec> mTabSpecs = new ArrayList<TabHost.TabSpec>();
	...
	{
		if (mTabSpecs.size() > this.getCurrentTab())
			return mTabSpecs.get(this.getCurrentTab());
		else
			return null;
	}
